...
contract Marriage {
    ...
    /// modifier to check if lawyer is the one to call the function
    modifier isLawyer(){
    ...
    /// modifier for valid divorce
    modifier isDivorced(uint256 _registry){
        if(coupleData(_registry).status == MarriageStatus(0)) {
            throw;    
        }
        _;
    }
     ...
     */
    function divorced(uint256 _registry) isDivorced() public {
    ...
    function withdrawEther() public isLawyer() returns(bool) {
        ...
        lawyer.Fee = laywyer.Fee + feeCollected;
